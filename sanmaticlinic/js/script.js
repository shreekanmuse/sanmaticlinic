...
function initFormValidation() {
  ...
  forms.forEach(form => {
    ...
    // Real-time blur validation setup for appointment fields
    ...
    function validateField(input) {
      ...
      // Skip validating empty fields on tab-through to prevent annoying alerts on initial focus loss
      if (val === "") return "";
      ...
      if (input.id === 'form-name') {
        if (val.length < 2) {
          ...
          errorMsg = "Full Name should only contain alphabetic letters and spaces.";
        }
      } else if (input.id === 'form-phone') {
        if (!/^[1-9][0-9]{9}$/.test(val)) {
          errorMsg = "Mobile Number must be exactly 10 digits and cannot start with 0.";
        }
      } else if (input.id === 'form-email') {
        const emailHasUppercase = /[A-Z]/.test(val);
        const emailEndsWithCom = /\.com$/.test(val);
        const emailRegex = /^[a-z0-9._%+-]+@[a-z0-9.-]+\.com$/;

        if (emailHasUppercase) {
          errorMsg = "Email Address should not contain uppercase letters.";
        } else if (!emailEndsWithCom) {
          errorMsg = "Email Address must end with .com.";
        } else if (!emailRegex.test(val)) {
          errorMsg = "Please enter a valid lowercase email format (e.g., name@domain.com).";
        }
      } else if (input.id === 'form-message') {
        if (val.length < 5) {
          errorMsg = "Ailment/therapy details must be at least 5 characters long.";
        }
      }

      // Clear any existing custom error labels
      const existingLabel = input.parentNode.querySelector('.field-error-label');
      if (existingLabel) {
      ...
      if (errorMsg) {
        ...
        label.style.cssText = 'color: #ff5c5c; font-size: 0.8rem; margin-top: 0.35rem; font-family: var(--font-body); display: block;';
    ...
    [nameField, phoneField, emailField, messageField].forEach(input => {
      if (input) {
        input.addEventListener('blur', () => {
          ...
          if (error) {
            alert(error);
            // Refocus back to the invalid input
            setTimeout(() => input.focus(), 10);
          } else {
            // If valid, make sure we remove the error label
            const existingLabel = input.parentNode.querySelector('.field-error-label');
            if (existingLabel) {
              existingLabel.remove();
            }
    ...
    form.addEventListener('submit', (e) => {
      ...
      // Clear all previous invalid classes and labels
      form.querySelectorAll('.invalid').forEach(input => input.classList.remove('invalid'));
      form.querySelectorAll('.field-error-label').forEach(label => label.remove());
